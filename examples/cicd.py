...
async def list_workflows():
    ...
    async with SourceCraftClient(api_token=api_token) as client:
        try:
            ...
            # List workflows
            workflows = await client.cicd.list_workflows(owner, repo_name, page=1, per_page=10)
...
async def manage_workflow_runs():
    ...
    async with SourceCraftClient(api_token=api_token) as client:
        try:
            ...
            runs = await client.cicd.list_workflow_runs(owner, repo_name, page=1, per_page=5)
            print(f"Found {runs.total} workflow runs in {owner}/{repo_name}:")
            for run in runs.data:
                print(f"  - Run #{run.id}: {run.name}")
                print(f"    Status: {run.state.value}, Conclusion: {run.conclusion.value if run.conclusion else 'pending'}")
            ...
            if runs.data:
                ...
                run_details = await client.cicd.get_workflow_run(owner, repo_name, run_id)
...
async def manage_pipelines():
    ...
    async with SourceCraftClient(api_token=api_token) as client:
        try:
            ...
            # List pipelines
            pipelines = await client.cicd.list_pipelines(owner, repo_name, page=1, per_page=5)
...
async def manage_artifacts():
    ...
    async with SourceCraftClient(api_token=api_token) as client:
        try:
            ...
            # List all artifacts
            artifacts = await client.cicd.list_artifacts(owner, repo_name, page=1, per_page=5)
            ...
            if artifacts.data:
                ...
                print(f"Note: Uncomment download code to actually download artifact {artifact_id}")
