...
async def list_repositories():
    ...
    async with SourceCraftClient(api_token=api_token) as client:
        try:
            ...
            for repo in repos.data[:5]:  # Show first 5
            ...
            if repos.has_next:
                print(f"... and {repos.total - len(repos.data)} more repositories")
...
async def create_and_update_repository():
    ...
    async with SourceCraftClient(api_token=api_token) as client:
        try:
            ...
            print(f"✓ Updated repository visibility to: {updated_repo.visibility.value}")
